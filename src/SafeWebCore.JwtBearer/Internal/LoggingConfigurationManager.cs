...
internal sealed partial class LoggingConfigurationManager : BaseConfigurationManager, IConfigurationManager<OpenIdConnectConfiguration>
{
    ...
    private async Task<BaseConfiguration> GetBaseConfigurationCoreAsync(CancellationToken cancel)
    {
        ...
        catch (Exception ex) when (ShouldLogFailure())
        {
            ...
            {
                LogPermanentFailure(_logger, address, ex); // NOSONAR: implementation generated by the LoggerMessage source generator
            }
            else
            {
                LogTransientFailure(_logger, address, ex); // NOSONAR: implementation generated by the LoggerMessage source generator
            }
    ...
    public Task<OpenIdConnectConfiguration> GetConfigurationAsync(CancellationToken cancel)
        => GetConfigurationCoreAsync(cancel);

    /// <inheritdoc />
    public Task<OpenIdConnectConfiguration> GetConfigurationAsync()
        => GetConfigurationAsync(CancellationToken.None);

    private async Task<OpenIdConnectConfiguration> GetConfigurationCoreAsync(CancellationToken cancel)
    {
        BaseConfiguration configuration = await GetBaseConfigurationCoreAsync(cancel).ConfigureAwait(false);
    ...
    [LoggerMessage(EventId = 1, Level = LogLevel.Error, Message = "OpenID Connect metadata for '{MetadataAddress}' could not be retrieved (permanent configuration error, HTTP 4xx). Requests fail closed until this is fixed.")]
    static partial void LogPermanentFailure(ILogger logger, string metadataAddress, Exception exception); // NOSONAR: implementation generated by the LoggerMessage source generator

    [LoggerMessage(EventId = 2, Level = LogLevel.Warning, Message = "OpenID Connect metadata for '{MetadataAddress}' could not be retrieved (transient failure). Requests keep failing closed until the metadata endpoint is reachable again.")]
    static partial void LogTransientFailure(ILogger logger, string metadataAddress, Exception exception); // NOSONAR: implementation generated by the LoggerMessage source generator
}